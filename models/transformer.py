...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head, attn_pdrop=0.0, resid_pdrop=0.0):
        ...
        self.attn = MultiheadAttention(n_embd, n_head, attn_pdrop, batch_first=True)
        self.cross_attn = MultiheadAttention(
            n_embd, n_head, attn_pdrop, batch_first=True
        )
        ...
        self.ls1 = LayerScale(n_embd, init_values=1e-5)
        self.ls2 = LayerScale(n_embd, init_values=1e-5)
        self.ls3 = LayerScale(n_embd, init_values=1e-5)

    def forward(self, x, condition=None):
        x_out, _ = self.attn(self.ln1(x), self.ln1(x), self.ln1(x))
        x = x + self.ls1(x_out)
        x_out, _ = self.cross_attn(self.ln2(x), condition, condition)
        x = x + self.ls2(x_out)
        x_out = self.mlp(self.ln3(x))
        x = x + self.ls3(x_out)
        return x
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.proj_concat = nn.Linear(n_embd * 2, n_embd)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, t, x_prev=None, condition=None):
        """
        ----------------------------------------------
        N = batch size
        ...
        if self.use_global_residual:
            ...
            inp = [x, t_embedding]
        inp = torch.cat(inp, 1)
        output = self.decoder(inp, condition)
