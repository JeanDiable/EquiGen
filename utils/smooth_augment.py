...
def precompute_distances(cities_matrix):
    distances = torch.cdist(
        cities_matrix, cities_matrix, p=1
    )  # p=1 indicates L1 norm, which is equivalent to Manhattan distance.
    return distances
...
def shp(cities_matrix):
    ...
    optimized_tour = (
        optimized_tour[:dummy_node_index] + optimized_tour[dummy_node_index + 1 :]
    )
...
class PermutationManager:
    ...
    def compute_permute_dict(self):
        DG = tp.DependencyGraph().build_dependency(
            self.model, example_inputs=self.example_tensor
        )
        permute_dict = {}
...
class INRPermutationManager(PermutationManager):
    def __init__(self, inr_params, example_tensor):
        self.inr_params = inr_params
        self.example_tensor = example_tensor

    def compute_permute_dict(self):
        permute_dict = {}

        # weights = [v for k, v in self.inr_params.items() if 'weight' in k]
        # biases = [v for k, v in self.inr_params.items() if 'bias' in k]

        weights = tuple(
            [v.permute(1, 0) for w, v in self.inr_params.items() if "weight" in w]
        )

        biases = tuple([v for w, v in self.inr_params.items() if "bias" in w])

        full_city = torch.cat([w.view(w.size(0), -1) for w in weights], dim=1)
        order = self.shp(full_city)

        for i, (k, v) in enumerate(self.inr_params.items()):
            if 'weight' in k:
                permute_dict[k] = {'out_channels': order}
            elif 'bias' in k:
                permute_dict[k] = {'out_channels': order}

        return permute_dict

    def apply_permutations(self, permute_dict):
        new_inr_params = {}
        for name, param in self.inr_params.items():
            if name in permute_dict:
                for mode, order in permute_dict[name].items():
                    if 'weight' in name:
                        new_inr_params[name] = param[order]
                    elif 'bias' in name:
                        new_inr_params[name] = param[order]
            else:
                new_inr_params[name] = param
        return new_inr_params

    def __call__(self):
        permute_dict = self.compute_permute_dict()
        return self.apply_permutations(permute_dict)


def smooth_augment_inr(inr_params, example_tensor):
    permutation_manager = INRPermutationManager(inr_params, example_tensor)
    smoothed_inr_params = permutation_manager()
    return smoothed_inr_params
